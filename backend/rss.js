...
app.post("/email", async (req, res) => {
  let follows = [];
  await axios
    .get("http://localhost:8081/follow?username=" + req.body.username)
    .then((data) => (follows = data.data))
  ...
  follows.map(async (itemt) => {
    ...
    } else if (itemt.follow == "eats") {
      const feed = await parser.parseURL(eats);
      feed.items.forEach((item) => {
        articles.push({ item });
      });
    } else if (itemt.follow == "network") {
...
<h3><a target='_blank' href=${item.item.link}>${item.item.title}</a></h3>
     <p>${new Date(item.item.pubDate).toLocaleDateString(
       "en-GB",
       formatted
     )}</p>
</div>`;
   ...
   .join("")}
    ...
    const now = new Date();
    const timestamp =
      new Date(now.getFullYear(), now.getMonth(), now.getDate(), 9).getTime() /
      1000;
    ...
    axios
      .post("https://api.sendinblue.com/v3/smtp/email", emailData, {
        headers: {
          "api-key":
            "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",
        },
      })
...
app.listen(process.env.PORT || 4000, () => {
  console.log(`server technology on http://localhost:4000/`);
});
